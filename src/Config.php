...
class Config
{
    ...
    {
        if (!isset($cfg['token'])) {
            throw new BotException('Token not defined!');
        }
        ...
        self::$cfg = [
            ...
            'abs_path' => $cfg['abs_path'] ?? throw new BotException('No absolute path has been defined'),
        ];
    }
    ...
    {
        if (!self::has($name)) {
            throw new BotException("Parameter not found: $name");
        }
        self::$cfg[$name] = $value;
    }
    ...
    private static function validateToken(string $token): string
    {
        if (!preg_match('/(\d+):[\w\-]+/', $token)) {
            throw new BotException("Invalid Token!");
        }
        return $token;
    }
